...
class D3Chart {
  ...
  update(gender) {
    ...
    // DATA JOIN - tell d3 what the new data is
    const rects = vis.svg.selectAll("rect")
                     .data(vis.data)

    // EXIT - remove elements not in data
    rects.exit()
         .transition()
         .duration(500)
         .attr("height", 0)
    ...
    // UPDATE - on group selection, update elements that still exist in data and on screen
    rects.transition()
         .duration(500)
         .attr("x", d => x(d.name))
         .attr("y", d => y(d.height))
    ...
    // ENTER - add new data to screen
    rects.enter()
         .append("rect")
         .attr("x", d => x(d.name))
         .attr("width", x.bandwidth)
         .attr("fill", "grey")
      // set y to "rise up" rather than fall down
         .attr("y", HEIGHT)
      // what to transition has to come after the transition is set
         .transition()
         .duration(500)
